...
def lambda_handler(event, context):
    ...
    try:
        ...
        ec2_instance_id = ssm.get_parameter(Name='/myapp/ec2-instance-id')['Parameter']['Value']
        print(f"EC2 Instance ID: {ec2_instance_id}")
        
        # Get destination path from Parameter Store
        destination_path = ssm.get_parameter(Name='/myapp/ec2-destination-path')['Parameter']['Value']
        ...
        if status == 'Success':
            ...
            print(f"Error details: {result.get('StandardErrorContent', 'No error details')}")
